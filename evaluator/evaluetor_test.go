...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{"name": "Monky"}[fn(x) { x }];`, fmt.Sprintf("unusable as hash key: FUNCTION")},
	}
	...
	for _, tt := range tests {
		...
		if errObj.Message != tt.expectedMessage {
			t.Errorf("%v: wrong error message. expected=%q, got=%q", tt.input, tt.expectedMessage, errObj.Message)
		}
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("four")`, 4},
		{`len("hello world")`, 11},
		//{`len(1)`, fmt.Sprintf("argument to %q not supported, got %s", object.BuiltinFuncNameLen, object.IntegerObj)},
